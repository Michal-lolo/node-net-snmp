...
var MIB = function (dir) {
    return ({
                                ...
                                if (Symbols[i + 1].indexOf('{') == 0) {
                                    ...
                                    if (Symbols[i - 1] == 'OBJECT IDENTIFIER') {
                                        ...
                                        while (c1 < (i - 1)) {
                                            ...
                                            if (key === "INDEX" && !keychain.includes(key)) {
                                                keychain.push('INDEX');
                                            }
                                ...
                                while (Symbols[tmp] != ';') {
                                    if (Symbols[tmp] == 'FROM') {
                                        ...
                                        if (!this.Modules[ImportModule]) {
                                            ...
                                            const err = new Error(ModuleName + ': Can not find ' + ImportModule);
                                            err.moduleName = ImportModule;
                                            throw err;
                                        }
        ...
        OID: function (OBJECT_IDENTIFIER, ID, ObjectName, OD, callback) {
            ...
            // Handle predefined OIDs from SNMPv2-SMI
            if (parent == 'enterprises') {
                let midID = ['1', '3', '6', '1', '4', '1']; // enterprises = 1.3.6.1.4.1
                let midOD = ['iso', 'org', 'dod', 'internet', 'private', 'enterprises'];
                for (let entry of members) {
                    let match = entry.match(/(.*)\((.+)\)$/);
                    if (match) {
                        midID.push(match[2]);
